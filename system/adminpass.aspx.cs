...
public partial class admin_pass : System.Web.UI.Page
{
    ...
    {   
        ...
        {
            //根据用户编号和原密码得到用户信息
            SqlDataReader sdr = DbHelperSQL.ExecuteReader("select * from admin where lname='" + Session["bh"].ToString() + "' and pwd='" + txt_pwd.Text + "'");

            //判断原密码是否正确
            if (sdr.Read())
            {
                //更新新密码
                DbHelperSQL.ExecuteSql("update admin set pwd='" + TextBox1.Text + "' where lname='" + Session["bh"].ToString()+ "'");
