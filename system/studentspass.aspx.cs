...
public partial class students_pass : System.Web.UI.Page
{
    ...
    {   
        ...
        {
            //根据用户编号和原密码得到用户信息
            SqlDataReader sdr = DbHelperSQL.ExecuteReader("select * from students where sno='" + Session["bh"].ToString() + "' and password='" + txt_pwd.Text + "'");

            //判断原密码是否正确
            if (sdr.Read())
            {
                //更新新密码
                DbHelperSQL.ExecuteSql("update students set password='" + TextBox1.Text + "' where sno='" + Session["bh"].ToString()+ "'");
